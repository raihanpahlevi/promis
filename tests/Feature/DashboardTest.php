...
class DashboardTest extends TestCase
{
    ...
    public function test_area_breakdown_shares_the_same_ring_denominator_across_variants(): void
    {
        $kantor = Kantor::create(['kode' => 'K1', 'nama' => 'Kantor Satu']);
        $ring1 = Poi::AREA_OPTIONS[0];
        $ring2 = Poi::AREA_OPTIONS[1];
        $ring3 = Poi::AREA_OPTIONS[2];
        // Ring 1: 2 POI (1 BNI, 1 Non); Ring 2: 1 POI (BNI); Ring 3: 1 POI (Non) — all included.
        ...
        $this->poi($kantor, ['area' => $ring3, 'status_mitra' => 'Bukan Nasabah BNI']);

        $admin = User::factory()->admin()->create(['force_password_change' => false]);
        $response = $this->actingAs($admin)->get('/dashboard?kantor='.$kantor->id);

        $response->assertOk();
        $response->assertViewHas('area', function ($area) use ($ring1, $ring2, $ring3) {
            // Denominator is the POI total (4), shared by every variant.
            return $area['all'][$ring1]['total'] === 2
                && $area['all'][$ring1]['persen'] === 50.0
                && $area['all'][$ring2]['total'] === 1
                && $area['all'][$ring3]['total'] === 1
                && $area['all'][$ring3]['persen'] === 25.0
